...
void* test_sequential_alloc_my_free(void* arg) {
...
void* test_batch_alloc_my_free(void* arg) {
    thread_data_t* data = (thread_data_t*)arg;
    double start_time = get_time_us();
...
void* test_random_alloc_my_free(void* arg) {
    thread_data_t* data = (thread_data_t*)arg;
    double start_time = get_time_us();
...
int main() {
    ...
    for (int i = 0; i < NUM_THREADS/2; i++) {
        ...
        pthread_create(&threads[i], NULL, test_sequential_alloc_my_free, &thread_data[i]);
    }
    ...
    void* (*test_functions[])(void*) = {
        test_sequential_alloc_my_free,
        test_batch_alloc_my_free,
        test_random_alloc_my_free,
